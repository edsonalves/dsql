...
import dsql.impl.SourceImpl;

/**
 * @generated
 */
public class SourceNameEditPart extends CompartmentEditPart implements ITextAwareEditPart {
	...
	*/
	protected void setLabelTextHelper(IFigure figure, String text) {
		String alias = ((SourceImpl) ((Node) this.getModel()).getElement()).getAlias();
	...
	*/
	public IParser getParser() {
		if (parser == null) {
			parser = DsqlParserProvider.getParser(DsqlElementTypes.Source_3049, getParserElement(),
					DsqlVisualIDRegistry.getType(dsql.diagram.edit.parts.SourceNameEditPart.VISUAL_ID));
		}
		return parser;
	}
	...
	*/
	protected void handleNotificationEvent(Notification event) {
		if (event.getEventType() == Notification.SET) {
			if (event.getNotifier() instanceof SourceImpl) {
				if( event.getFeature() instanceof EAttribute ){
					//TableImpl obj = (TableImpl) event.getNotifier();
					EAttribute atributo = (EAttribute) event.getFeature();
					if (atributo.getName().equalsIgnoreCase("name") || 
