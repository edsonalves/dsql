...
public class QueryNameEditPart extends CompartmentEditPart implements ITextAwareEditPart {
	...
	*/
	protected void setLabelTextHelper(IFigure figure, String text) {
		int limitOffset = ((QueryImpl) ((Node) this.getModel()).getElement()).getLimitOffset();
		int limitRowCount = ((QueryImpl) ((Node) this.getModel()).getElement()).getLimitRowCount();
		
		SetQuantifier setQuantifier = ((QueryImpl) ((Node) this.getModel()).getElement()).getSetQuantifier();
		
	
		if(limitOffset != 0 || limitRowCount != 0){
			text+= "  LIM "+limitOffset + " - "+limitRowCount;
		}
...
//			wl.setTextWrap(true);
//			System.out.println(wl.getTextBounds());
//			
//		}
	...
	*/
	public IParser getParser() {
		if (parser == null) {
			parser = DsqlParserProvider.getParser(DsqlElementTypes.Query_2003, getParserElement(),
					DsqlVisualIDRegistry.getType(dsql.diagram.edit.parts.QueryNameEditPart.VISUAL_ID));
		}
		return parser;
	}
	...
	*/
	protected void handleNotificationEvent(Notification event) {
		if (event.getEventType() == Notification.SET) {
			if (event.getNotifier() instanceof QueryImpl) {
				if( event.getFeature() instanceof EAttribute ){
					EAttribute atributo = (EAttribute) event.getFeature();
					if (atributo.getName().equalsIgnoreCase("limitOffset") || 
							atributo.getName().equalsIgnoreCase("limitRowCount") ) {
						refreshLabel();
					}
				}
			}
